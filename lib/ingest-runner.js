...
// Must match the model used by getQueryEmbedding in index.js
// (gemini-embedding-001 = 3072-dim). Mixing models leaves chunks
// uncomparable with queries — they'd never surface in retrieval.
...
function appendChunksToKB(chunks) {
  // Existing KB is an ARRAY of chunks. We preserve that shape.
  let kb = [];
  if (fs.existsSync(KB_FILE)) {
    const raw = JSON.parse(fs.readFileSync(KB_FILE, 'utf8'));
    kb = Array.isArray(raw) ? raw : Object.values(raw);
  }
  // Give each chunk a stable id similar to the existing schema.
  const startId = kb.length;
  chunks.forEach((c, i) => {
    if (!c.id) c.id = `${c.book.toLowerCase()}_${c.chapter}_${startId + i}`;
    kb.push(c);
  });
