...
eq(A.VARNA_BY_RASHI.length, 12, 'VARNA_BY_RASHI covers 12 rashis');
...
// ── symmetry ──
// Two people running the same check from their own phones must see the same
// number. The classical Varna and Gana rules are directional (bride vs
// groom); we deliberately symmetrise them, and this asserts it holds across
// every nakshatra pair AND across rashi pairs.
{
  let asym = 0;
  for (const na of A.NAKSHATRAS) {
    for (const nb of A.NAKSHATRAS) {
      const x = A.ashtakoot({ nakshatra: na, rashi: 'Aries' }, { nakshatra: nb, rashi: 'Leo' });
      const y = A.ashtakoot({ nakshatra: nb, rashi: 'Leo' }, { nakshatra: na, rashi: 'Aries' });
      if (x.total !== y.total) asym++;
    }
  }
  eq(asym, 0, 'symmetric across all 729 nakshatra pairs');
}
{
  let asym = 0;
  for (const ra of A.RASHIS) {
    for (const rb of A.RASHIS) {
      const x = A.ashtakoot({ nakshatra: 'Rohini', rashi: ra }, { nakshatra: 'Mula', rashi: rb });
      const y = A.ashtakoot({ nakshatra: 'Mula', rashi: rb }, { nakshatra: 'Rohini', rashi: ra });
      if (x.total !== y.total) asym++;
    }
  }
  eq(asym, 0, 'symmetric across all 144 rashi pairs');
}

// ── Varna uses the rashi element, not the nakshatra ──
{
  const libra = A.ashtakoot({ nakshatra: 'Vishakha', rashi: 'Libra' },
                            { nakshatra: 'Rohini', rashi: 'Gemini' });
  // Libra and Gemini are both air -> both Shudra -> equal rank -> 1 point
  eq(libra.kutas.find(k => k.label === 'Varna').points, 1, 'air+air -> Varna 1');
  const gap = A.ashtakoot({ nakshatra: 'Vishakha', rashi: 'Libra' },
                          { nakshatra: 'Rohini', rashi: 'Cancer' });
  // Shudra(1) vs Brahmin(4) -> gap of 3 -> 0
  eq(gap.kutas.find(k => k.label === 'Varna').points, 0, 'shudra+brahmin -> Varna 0');
}
