...
// ── free-tier teaser lines (deterministic, no LLM) ───────────
//
// This is what a FREE user sees every single day, so it carries the
// whole daily-return hook. A single line per verdict meant one user saw
// the identical sentence every neutral day forever — which trains them
// to stop looking. These vary by mulank (so it feels like theirs) and
// rotate by date (so it changes), while staying pure arithmetic: same
// inputs, same output, no LLM call, works during a provider outage.
//
// SCOPE RULE (same as the LLM's): describe QUALITY and THEME only.
// Never a specific event, amount, or outcome. Read the honesty note at
// the top of this file before adding lines.
//
// `{period}` is substituted with day / week / month, so one bank serves
// all three periods — a locked WEEKLY reading must not say "day".
const PERIOD_WORD = { daily: 'day', weekly: 'week', monthly: 'month' };

// Fallback if a mulank/verdict pair is ever missing — never throw at a
// user for want of a teaser.
const GENERIC_TEASER = {
  favourable: 'A supportive {period} — lean into it.',
  neutral: 'A steady, ordinary {period} — keep to your plans.',
  caution: 'Go gently — be patient, avoid rushed decisions.',
};

const TEASERS = {
  1: { // Sun — The Leader
    favourable: [
      'Your {period} backs bold moves — step forward and lead it.',
      'A sun-lit {period}: your initiative lands well. Start the thing.',
      'A strong {period} for taking charge and being seen.',
    ],
    neutral: [
      'A steady {period} — hold your direction, no need to push.',
      'No need to force anything this {period}. Keep your own counsel.',
      'An even {period}: lead quietly, let results speak.',
    ],
    caution: [
      'Go gently this {period} — ego costs more than it wins.',
      'A softer {period}. Listen first; command later.',
      'Hold back this {period} — patience beats pushing.',
    ],
  },
  2: { // Moon — The Diplomat
    favourable: [
      'A warm {period} for people — reach out, mend, connect.',
      'Feelings run clear this {period}. Trust what you sense.',
      'A gentle, supportive {period} at home and in close ties.',
    ],
    neutral: [
      'A quiet {period} — steady moods, no big waves.',
      'An ordinary {period}. Let plans move at their own pace.',
      'Calm water this {period} — nothing needs deciding yet.',
    ],
    caution: [
      "Guard your mood this {period} — don't take things to heart.",
      'A tender {period}. Rest more, react less.',
      'Go gently — this {period} asks for patience with people.',
    ],
  },
  3: { // Jupiter — The Optimist
    favourable: [
      'A generous {period} for learning, teaching and honest advice.',
      'Jupiter favours this {period} — say yes to growth.',
      'A good {period} to plan, study, or guide someone.',
    ],
    neutral: [
      'A level {period} — keep learning, expect no fireworks.',
      'Steady progress this {period}. Small steps count.',
      "An even {period}: revise, review, don't over-promise.",
    ],
    caution: [
      'Go gently — this {period} punishes over-confidence.',
      "Promise less this {period}; finish what's already open.",
      'A careful {period}. Advice given now may not land.',
    ],
  },
  4: { // Rahu — The Rebel
    favourable: [
      'An inventive {period} — unusual routes work better than usual ones.',
      'A good {period} to fix, rebuild, or try the odd idea.',
      'A resourceful {period}. Your different angle pays.',
    ],
    neutral: [
      'A workmanlike {period} — grind through, nothing dramatic.',
      'A steady {period}. Keep systems simple, avoid sudden turns.',
      "An ordinary {period}: hold your routine, it's working.",
    ],
    caution: [
      'Go gently — this {period} rewards patience over sudden moves.',
      "A restless {period}. Don't decide anything in a hurry.",
      'Avoid big changes this {period}; let the dust settle.',
    ],
  },
  5: { // Mercury — The Communicator
    favourable: [
      'A quick, talkative {period} — conversations and deals move.',
      'Mercury favours this {period}: write, pitch, negotiate.',
      'A nimble {period} for travel, trade and good news.',
    ],
    neutral: [
      'A busy but even {period} — clear your messages, nothing more.',
      'A steady {period}. Keep talking, expect ordinary answers.',
      "An average {period}: tidy the details, don't overthink.",
    ],
    caution: [
      'Go gently — this {period} scatters focus. Slow your words.',
      'Re-read anything you send this {period}.',
      'A noisy {period}. Say less; double-check the details.',
    ],
  },
  6: { // Venus — The Nurturer
    favourable: [
      'A lovely {period} for people, beauty and small comforts.',
      'Venus warms this {period} — enjoy company, make peace.',
      'A soft, pleasant {period}. Good for love and for making things.',
    ],
    neutral: [
      'A comfortable {period} — nothing pressing, nothing missing.',
      'An easy {period}. Tend what you already have.',
      'A steady {period}: keep the peace, spend carefully.',
    ],
    caution: [
      'Go gently — this {period} tempts overspending and over-giving.',
      'A delicate {period} for close ties — be honest, not agreeable.',
      'Watch comfort spending this {period}.',
    ],
  },
  7: { // Ketu — The Seeker
    favourable: [
      'A deep {period} — solitude, study and insight come easily.',
      'Ketu favours this {period}: look inward, research, reflect.',
      'A quiet, clarifying {period}. Trust your intuition.',
    ],
    neutral: [
      'A still {period} — little noise, little movement.',
      'An inward {period}. Let some questions sit unanswered.',
      'A steady {period}: keep your own company, no rush.',
    ],
    caution: [
      'Go gently — this {period} pulls you too far inward.',
      "A foggy {period}. Don't withdraw from the people who matter.",
      'Stay grounded this {period}; over-thinking clouds it.',
    ],
  },
  8: { // Saturn — The Achiever
    favourable: [
      'A solid {period} for long work — discipline pays off now.',
      'Saturn steadies this {period}: build, organise, commit.',
      'A productive {period}. Slow effort moves real things.',
    ],
    neutral: [
      'A plain, working {period} — show up and keep going.',
      'A steady {period}. Progress is quiet but real.',
      'An ordinary {period}: hold the routine, expect no shortcuts.',
    ],
    caution: [
      'Go gently — this {period} brings delays. Patience, not force.',
      'A heavy {period}. Do less, and do it properly.',
      "Expect friction this {period}; don't take it personally.",
    ],
  },
  9: { // Mars — The Warrior
    favourable: [
      'A high-energy {period} — act, compete, finish something.',
      'Mars fires this {period}: push hard, you have the fuel.',
      'A bold {period} for action and honest effort.',
    ],
    neutral: [
      'A steady {period} — energy is fine, nothing to fight.',
      'An even {period}. Spend the effort on routine work.',
      'An ordinary {period}: keep moving, pick no battles.',
    ],
    caution: [
      'Go gently — this {period} sparks temper. Cool down first.',
      'A sharp {period}. Avoid arguments and rushed decisions.',
      'Hold your fire this {period}; patience wins it.',
    ],
  },
};

// Teaser for (mulank, verdict) on a given date. Deterministic: rotates
// on the day-of-month, offset by the mulank so two people with
// different numbers on the same day do not read the same sentence.
function teaserFor(m, verdict, date, period) {
  const word = PERIOD_WORD[period] || 'day';
  const bank = (TEASERS[m] || {})[verdict];
  if (!bank || !bank.length) {
    const g = GENERIC_TEASER[verdict] || GENERIC_TEASER.neutral;
    return g.replace(/\{period\}/g, word);
  }
  let day;
  try { day = parseCivilDate(date).day; } catch (e) { day = 1; }
  const idx = (day + m) % bank.length;
  return bank[idx].replace(/\{period\}/g, word);
}

module.exports = {
  PLANET, FRIENDSHIP, PROFILE, TEASERS, teaserFor,
