...
// Rate a single day for a person. Pure, deterministic.
...
// Aggregate an inclusive span of civil dates [start, end] into one
// verdict. Used by week (7 days) and month.
function spanRating(dob, startDate, endDate) {
  ...
  for (let t = new Date(start); t <= end; t.setDate(t.getDate() + 1)) {
    const r = dayRating(dob, new Date(t));
    days.push(r);
    sum += r.score;
  ...
  // Best / worst days (first extreme of each).
...
// Week starting at `startDate` (7 inclusive days).
function weekRating(dob, startDate) {
  const s = parseCivilDate(startDate);
  const start = new Date(s.year, s.month - 1, s.day);
  const end = new Date(start); end.setDate(end.getDate() + 6);
  return spanRating(dob, start, end);
}

// Whole calendar month (year, month 1–12).
