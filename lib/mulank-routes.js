...
function registerMulankRoutes(app, deps) {
  ...
  app.post('/mulank/ask', async (req, res) => {
    ...
    try {
      ...
      const tgt = targetDate(date);
      const built = prompt.askPrompt(birthDate, tgt, question.slice(0, 500), (language || 'en').toString().slice(0, 12));
      let answer = null;
...
module.exports = { registerMulankRoutes, canSeeReading, weekMonday, preGenerateMulank };
