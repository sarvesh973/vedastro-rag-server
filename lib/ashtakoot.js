...
// Per-nakshatra attributes, indexed 0-26 in the order above.
const VARNA = ['Vaishya', 'Kshatriya', 'Brahmin', 'Shudra', 'Shudra', 'Shudra',
  'Vaishya', 'Kshatriya', 'Brahmin', 'Shudra', 'Brahmin', 'Kshatriya',
  'Vaishya', 'Shudra', 'Shudra', 'Brahmin', 'Shudra', 'Vaishya',
  'Kshatriya', 'Brahmin', 'Kshatriya', 'Shudra', 'Shudra', 'Shudra',
...
function varnaKuta(a, b) {
  const va = VARNA[a.nak], vb = VARNA[b.nak];
  // Traditional rule: full point unless the bride's varna outranks the groom's.
  const points = VARNA_RANK[vb] <= VARNA_RANK[va] ? 1 : 0;
  return {
...
function ganaKuta(a, b) {
  ...
  else if (ga === 'Rakshasa' && gb === 'Manushya') points = 0;
  else if (ga === 'Manushya' && gb === 'Rakshasa') points = 1;
  else points = 0; // deva vs rakshasa
  return {
...
module.exports = {
  NAKSHATRAS, RASHIS, VARNA, YONI, GANA, NADI, VASHYA,
  ashtakoot,
  // exported for tests:
  nakIndex, rashiIndex,
};
