...
module.exports = [
...

  {
    ...
    predicate: (c) => {
      ...
      const active = [l2, l11].filter(x => x && (md === x || ad === x));
      if (active.length === 0) return { matched: false };
      return { matched: true, intensity: active.length === 2 ? 9 : 7, evidence: `Wealth-house lord(s) in dasha: ${active.join(', ')} (current ${md}/${ad})` };
    },
  },

  {
    ...
    prediction: { polarity: 'positive', text: 'Jupiter dasha — wealth-karaka period. Expansion of resources, returning investments, education-related income.', timeframe: 'currentDasha' },
    predicate: (c) => {
      const md = c.dasha && c.dasha.mahadasha;
