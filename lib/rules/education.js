...
module.exports = [
  ...
  },
  {
    ...
    prediction: { polarity: 'positive', text: 'Mercury dasha — peak intellectual period. Best time for new learning, exams, certifications, language acquisition.', timeframe: 'currentDasha' },
    predicate: (c) => {
      const md = c.dasha && c.dasha.mahadasha;
      const ad = c.dasha && c.dasha.antardasha;
      if (md !== 'Mercury' && ad !== 'Mercury') return { matched: false };
      return { matched: true, intensity: md === 'Mercury' ? 7 : 6, evidence: `Mercury active in dasha ${md}/${ad}` };
    },
  },
  {
    ...
    prediction: { polarity: 'positive', text: 'Jupiter dasha — favourable period for higher learning, returning to study, philosophical/spiritual research.', timeframe: 'currentDasha' },
    predicate: (c) => {
      const md = c.dasha && c.dasha.mahadasha;
