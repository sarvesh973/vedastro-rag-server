...
function evaluateDomain(chart, domain, opts = {}) {
  ...
  // NEGATIVE-PRESERVATION PASS — real-user feedback flagged that the app
  // never tells anyone there's a problem with their career (or wealth,
  // etc.). Root cause: the top-N cut by intensity routinely drops matched
  // negatives when several positives rank higher, so the LLM never sees a
  // negative rule and produces an all-rosy answer.
  //
  // Fix: any matched negative that DIDN'T make the top-N is appended back
  // in. We keep every matched negative — if the chart legitimately shows
  // 4 negative signals, the user hears all 4, not just the top one.
  // Positives can still get bumped to make room intellectually (the LLM
  // sees both), but the rendered output preserves rule count: top-N +
  // any remaining negatives.
  const includedIds = new Set(top.map(m => m.id));
  const extraNegatives = matched
    .filter(m => m.polarity === 'negative' && !includedIds.has(m.id));
  if (extraNegatives.length > 0) {
    top.push(...extraNegatives);
  }
  return top;
}
