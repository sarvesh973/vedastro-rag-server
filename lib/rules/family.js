...
module.exports = [
...

  {
    ...
    predicate: (c) => {
      const moonLayer = dashaActive(c, 'Moon');
      const l4 = lordOfHouse(c, 4);
      const l4Layer = l4 && dashaActive(c, l4);
      if (!moonLayer && !l4Layer) return { matched: false };
      const hits = [];
      if (moonLayer) hits.push(`Moon(${moonLayer})`);
      if (l4Layer) hits.push(`4L ${l4}(${l4Layer})`);
      const best = moonLayer || l4Layer;
      return { matched: true, intensity: dashaIntensity(best), evidence: `Active: ${hits.join(', ')}` };
    },
  },
  {
    ...
    prediction: { polarity: 'mixed', text: 'Sun or 9th-lord active in dasha — significant father-related period.', timeframe: 'currentDasha' },
    predicate: (c) => {
      const sunLayer = dashaActive(c, 'Sun');
      const l9 = lordOfHouse(c, 9);
      const l9Layer = l9 && dashaActive(c, l9);
      if (!sunLayer && !l9Layer) return { matched: false };
      const hits = [];
      if (sunLayer) hits.push(`Sun(${sunLayer})`);
      if (l9Layer) hits.push(`9L ${l9}(${l9Layer})`);
      const best = sunLayer || l9Layer;
      return { matched: true, intensity: dashaIntensity(best), evidence: `Active: ${hits.join(', ')}` };
    },
  },
];
