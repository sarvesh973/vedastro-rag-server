...
module.exports = [
  ...
  // ============== EXPANDED NEGATIVES (added after polarity-imbalance audit) ==============
  // Real-user feedback: 'I have never seen an answer where it says theres
  // break or anything wrong with your career in this app.' Career had 26
  // positive vs 3 negative rules. These add 7 classical negative
  // configurations so charts that genuinely show career friction will
  // now produce honest negative answers, not forced optimism.

  {
    id: 'saturn_in_tenth_no_sasha',
    domain: 'career',
    source: { book: 'Phaladeepika', chapter: 6, verse: '14' },
    note: 'Saturn in 10th gives heavy responsibility and slow growth. Only forms Sasha Yoga (positive) if Saturn is dignified (own sign Capricorn/Aquarius or exalted in Libra). Otherwise it weighs the career down with delays and low recognition.',
    prediction: { polarity: 'negative', text: 'Career progress feels heavy and slow. Recognition arrives late and only after sustained effort. Authority figures may be demanding or critical. The work itself is real, but rewards lag behind input.', timeframe: 'lifetime' },
    predicate: (c) => {
      if (planetHouse(c, 'Saturn') !== 10) return { matched: false };
      // Sasha Yoga cancels this — Saturn dignified in 10th is positive.
      if (isOwnSign(c, 'Saturn') || isExalted(c, 'Saturn')) return { matched: false };
      const intensity = isDebilitated(c, 'Saturn') ? 8 : 7;
      return { matched: true, intensity, evidence: `Saturn in 10th (${planetSign(c, 'Saturn')})${isDebilitated(c, 'Saturn') ? ', debilitated' : ''}` };
    },
  },

  {
    id: 'mars_in_tenth_afflicting',
    domain: 'career',
    source: { book: 'BPHS', chapter: 49, verse: '38' },
    note: 'Mars in 10th can give Ruchaka Yoga if dignified, but otherwise brings workplace conflict, sudden setbacks, and risk of injury or lawsuit at work.',
    prediction: { polarity: 'negative', text: 'Workplace tends toward conflict and confrontation. Sudden setbacks, disputes with colleagues or superiors, or risk of legal trouble through work. Career advances often through force, not flow.', timeframe: 'lifetime' },
    predicate: (c) => {
      if (planetHouse(c, 'Mars') !== 10) return { matched: false };
      // Ruchaka Yoga cancels: Mars in own sign (Aries/Scorpio) or exalted (Capricorn).
      if (isOwnSign(c, 'Mars') || isExalted(c, 'Mars')) return { matched: false };
      const intensity = isDebilitated(c, 'Mars') ? 7 : 6;
      return { matched: true, intensity, evidence: `Mars in 10th (${planetSign(c, 'Mars')})${isDebilitated(c, 'Mars') ? ', debilitated' : ''}` };
    },
  },

  {
    id: 'tenth_lord_retrograde',
    domain: 'career',
    source: { book: 'Saravali', chapter: 28, verse: '31' },
    note: 'Retrograde 10th lord classically signifies repeated do-overs in career — going back to prior employers, redoing prior work, false starts.',
    prediction: { polarity: 'negative', text: 'Career path involves repetition and reversals. Old roles, old industries, or old employers tend to come back. Forward progress requires revisiting unfinished work first.', timeframe: 'lifetime' },
    predicate: (c) => {
      const l = lordOfHouse(c, 10); if (!l) return { matched: false };
      if (!isRetrograde(c, l)) return { matched: false };
      return { matched: true, intensity: 6, evidence: `10th lord ${l} retrograde` };
    },
  },

  {
    id: 'tenth_lord_combust',
    domain: 'career',
    source: { book: 'Phaladeepika', chapter: 6, verse: '21' },
    note: 'A combust 10th lord (within Sun\'s orb) loses its independent power — career visibility is eclipsed by the father/authority figure (Sun) or by institutional structure.',
    prediction: { polarity: 'negative', text: 'Professional visibility is muted. Work gets done but credit goes elsewhere — to a senior, to the firm, to the father. Personal identity in career struggles to emerge on its own.', timeframe: 'lifetime' },
    predicate: (c) => {
      const l = lordOfHouse(c, 10); if (!l) return { matched: false };
      if (!isCombust(c, l)) return { matched: false };
      return { matched: true, intensity: 7, evidence: `10th lord ${l} combust (within Sun's orb)` };
    },
  },

  {
    id: 'rahu_in_tenth',
    domain: 'career',
    source: { book: 'Phaladeepika', chapter: 6, verse: '24' },
    note: 'Rahu in 10th gives sudden unconventional rise but also sudden fall, especially in Rahu/Ketu dasha. Classical caution: success without ethical anchor is unstable.',
    prediction: { polarity: 'mixed', text: 'Capacity for sudden, unconventional career rises — non-traditional industries, foreign work, technology, media. The same energy that lifts can also drop suddenly, especially around Rahu sub-periods. Public scandal is a real risk if shortcuts are taken.', timeframe: 'lifetime' },
    predicate: (c) => {
      if (planetHouse(c, 'Rahu') !== 10) return { matched: false };
      const dlayer = dashaActive(c, 'Rahu');
      const intensity = dlayer ? dashaIntensity(dlayer) : 7;
      return { matched: true, intensity, evidence: `Rahu in 10th${dlayer ? ' + active in dasha' : ''}` };
    },
  },

  {
    id: 'sixth_lord_in_tenth',
    domain: 'career',
    source: { book: 'BPHS', chapter: 49, verse: '42' },
    note: 'The 6th house lord placed in the 10th brings the 6th-house themes (disputes, debts, opposition, illness) into the career arena.',
    prediction: { polarity: 'negative', text: 'The work environment carries chronic friction — disputes with colleagues, opposition from peers, or stress-driven health flare-ups during high-pressure periods. The role often involves managing other people\'s problems (legal, medical, debt, conflict resolution).', timeframe: 'lifetime' },
    predicate: (c) => {
      const sixL = lordOfHouse(c, 6); if (!sixL) return { matched: false };
      if (planetHouse(c, sixL) !== 10) return { matched: false };
      // If 6L is also 10L (e.g. Cancer ascendant: Saturn rules 7 and 8;
      // for Libra ascendant Jupiter rules 3 and 6 but not 10) — only
      // skip when 6L === 10L, which happens for specific ascendants.
      const tenL = lordOfHouse(c, 10);
      if (sixL === tenL) return { matched: false };
      return { matched: true, intensity: 6, evidence: `6th lord ${sixL} in 10th house` };
    },
  },

  {
    id: 'sade_sati_career_caution',
    domain: 'career',
    source: { book: 'Phaladeepika', chapter: 26, verse: '8' },
    note: 'Sade Sati is the 7.5-year transit of Saturn through 12th, 1st, and 2nd houses from natal Moon. Career often hits a compression phase during the middle leg (Saturn over natal Moon) — heavy responsibility, reduced rewards, frequent burnout.',
    prediction: { polarity: 'negative', text: 'Currently in or near Sade Sati period — Saturn is compressing the career karmically. Expect heavier workload with lower rewards, slower promotions, and reduced energy. The classical guidance is endurance: this phase teaches the difference between effort and outcome.', timeframe: 'transit' },
    predicate: (c) => {
      // The /chat handler injects ctx.transits via computeTransits.
      // sadeSati object is { active, phase } when transit data is present.
      const ss = c.transits && c.transits.sadeSati;
      if (!ss || !ss.active) return { matched: false };
      // Strongest in the middle ('peak') phase
      const peak = ss.phase === 'peak' || ss.phase === 'middle';
      return { matched: true, intensity: peak ? 8 : 6, evidence: `Sade Sati ${ss.phase || 'active'} phase` };
    },
  },
];
