...
// Get slow-planet positions AS OF a target date (default: today).
// For validation we pass the event date to check transits classically.
function getSlowPlanetsAt(targetDate) {
  const t = targetDate instanceof Date ? targetDate : new Date(targetDate);
  const at = getKundli(t, new Observer(28.6139, 77.209, 0));
  const out = {};
  for (const p of ['Saturn', 'Jupiter', 'Rahu', 'Ketu']) {
    const data = at.planets[p];
    if (data) {
      out[p] = {
...
// Backwards-compatible alias for the live-chat path.
function getCurrentSlowPlanets() { return getSlowPlanetsAt(new Date()); }
...
// ─── MAIN ENTRY ───
// Optional asOfDate lets the validation runner compute transits AT
// EVENT DATE rather than today. Live chat path passes nothing → today.
function computeTransits(natalChart, asOfDate) {
  ...
  const current = getSlowPlanetsAt(target);
